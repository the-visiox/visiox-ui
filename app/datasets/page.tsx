...
import {
  Plus, Search, Filter, Download, Tag, Image as ImageIcon,
  Grid, List, MoreVertical, CheckCircle2, Clock, X, Loader2,
} from "lucide-react";
...
function CreateDatasetModal({ projectList, onClose, onCreated }: CreateDatasetModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const created = await datasets.create({ project: projectId as number, name, description });
      onCreated(created);
    } catch (err: unknown) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-2">Description (optional)</label>
            <textarea
              value={description}
...
export default function DatasetsPage() {
  ...
  useEffect(() => {
    async function load() {
      const [dsRes, projRes] = await Promise.allSettled([datasets.list(), projects.list()]);
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-white/60 backdrop-blur-md p-2 rounded-2xl border border-stone-200 mb-8 sticky top-4 z-30">
          <div className="flex items-center gap-1 p-1">
            <button className="px-4 py-1.5 bg-stone-900 text-white rounded-xl text-xs font-bold flex items-center gap-2">
              <Grid className="w-3.5 h-3.5" /><span>Grid View</span>
            </button>
                    ...
                    <div className="px-4 pb-4">
                      ...
                      <div className="flex items-center gap-6 text-[10px] font-bold uppercase tracking-widest text-stone-400">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-stone-900">{dataset.media_count.toLocaleString()}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
