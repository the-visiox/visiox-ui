...
import { deployments, type InferenceEndpoint, type ModelRegistry } from "@/lib/api";
...
function NewEndpointModal({ registryList, onClose, onCreated }: NewEndpointModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!registryId) {
      setError("Select a model from registry.");
      return;
    }
    setSaving(true);
    setError("");
  ...
  return (
    <div
      className={["fixed inset-0 z-50 flex items-center justify-center bg-orange-950/20", "backdrop-blur-sm"].join(" ")}
    >
      <motion.div
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={saving || !name || !registryId}
              className={[
                "flex-1 py-3 bg-orange-500 text-white rounded-xl font-bold text-sm shadow-lg",
                "shadow-orange-500/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50",
...
export default function DeployPage() {
  ...
  useEffect(() => {
    async function load() {
      const [epRes, regRes] = await Promise.allSettled([deployments.listEndpoints(), deployments.listRegistry()]);
  ...
  return (
    <div className="relative flex-1 flex flex-col min-h-screen">
      ...
      <AnimatePresence>
        {showModal && (
          <NewEndpointModal registryList={registryList} onClose={() => setShowModal(false)} onCreated={handleCreated} />
        )}
      </AnimatePresence>
      ...
      <main className="flex-grow p-6 z-10">
        ...
        <div className="grid grid-cols-12 gap-8">
                  ...
                  <div className="flex gap-2">
                    {selectedEndpoint.status === "inactive" || selectedEndpoint.status === "error" ? (
                      <button
                        ...
                        className={[
                          "flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-xl text-xs font-bold",
                      ...
                      <button
                        ...
                        disabled={actionId === selectedEndpoint.id}
                        className={[
                          "flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 border border-red-200",
                          "rounded-xl text-xs font-bold hover:scale-105 transition-all disabled:opacity-50",
                        ].join(" ")}
                      >
                        {actionId === selectedEndpoint.id ? (
                          <Loader2 className="w-3.5 h-3.5 animate-spin" />
              ...
              {selectedEndpoint && (
                <div className="space-y-4">
                  ...
                  <div className="relative group">
                    <div
                      className={[
                        "flex items-center justify-between p-4 bg-stone-50 border border-stone-200 rounded-2xl",
                        "font-mono text-sm text-stone-600 overflow-hidden pr-20",
                      ].join(" ")}
                    >
                    ...
                    <button
                      ...
                      className={[
                        "absolute right-2 top-1/2 -translate-y-1/2 p-2.5 bg-white border border-stone-200",
                        "text-stone-600 rounded-xl hover:text-orange-500 transition-all shadow-sm",
                        "disabled:opacity-40",
                      ].join(" ")}
                    >
              ...
              {selectedEndpoint && (
                <div className="mt-8">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button
                      className={[
                        "text-[10px] font-bold text-orange-500 flex items-center gap-1",
                        "hover:underline",
                      ].join(" ")}
                    >
                  ...
                  <div
                    className={[
                      "bg-orange-50 rounded-2xl border border-orange-100 p-6 font-mono text-xs text-stone-700",
                      "leading-relaxed shadow-inner",
                    ].join(" ")}
                  >
                    <p>
                      <span className="text-orange-600">import</span> requests
                    </p>
                    <p className="mt-2 text-stone-500"># Predict using the REST endpoint</p>
                    <p>response = requests.post(</p>
                    <p className="pl-4">
                      <span className="text-orange-600">{`"${endpointUrl}"`}</span>,
                    </p>
                    <p className="pl-4">
                      headers={"{"}Authorization:{" "}
                      <span className="text-orange-600">{`"Bearer ${selectedEndpoint.auth_token}"`}</span>
                      {"}"}
                    </p>
                    <p className="pl-4">
                      json={"{"}image_url: <span className="text-orange-600">{`"https://..."`}</span>
                      {"}"}
                    </p>
                    <p>)</p>
              ...
              <div className="space-y-3">
                {loading ? (
                  ...
                  <div className="text-center py-8 text-stone-400 text-sm">
                    No endpoints yet. Deploy a model to get started.
                  </div>
                ) : (
                  endpoints.map((ep) => (
                    <button
                      ...
                      className={[
                        ...
                        selectedEndpoint?.id === ep.id ? "border-orange-500/50 shadow-md" : "border-stone-200",
                      ].join(" ")}
                    >
                        ...
                        <div className="flex items-center gap-2">
                          {ep.status === "inactive" ? (
                            <button
                              ...
                              className={[
                                "p-1.5 text-stone-400 hover:text-green-500 transition-colors",
                                "disabled:opacity-50",
                              ].join(" ")}
                            >
                              {actionId === ep.id ? (
              ...
              {[
                {
                  ...
                  status: "Active",
                  latency: "14ms",
                  desc: "Auto-scalable cloud clusters",
                },
                { title: "Edge Native", icon: Cpu, status: "Ready", latency: "2ms", desc: "NVIDIA Jetson / OAK-D" },
                {
                  ...
                  status: "Beta",
                  latency: "12ms",
                  desc: "iOS CoreML & Android TFLite",
                },
              ].map((item, i) => (
                <div
                  key={i}
                  className={[
                    "bg-white p-6 rounded-3xl border border-stone-200 shadow-sm hover:shadow-md",
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div
                      className={[
                        "w-10 h-10 bg-stone-50 rounded-xl flex items-center justify-center text-stone-400",
                        "group-hover:text-orange-500 group-hover:bg-orange-50 transition-all",
                      ].join(" ")}
                    >
                    ...
                    <span
                      className={[
                        "flex items-center gap-1.5 px-2 py-0.5 bg-green-50 text-[10px] font-bold text-green-700",
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
            <div
              className={[
                "bg-orange-50 rounded-3xl p-8 border border-orange-100 text-stone-900 shadow-2xl",
                "shadow-orange-100/70 relative overflow-hidden",
              ].join(" ")}
            >
              ...
              <div className="space-y-6">
                <div>
                  <div
                    className={[
                      "flex justify-between text-xs font-bold mb-2 uppercase tracking-widest",
                      "text-stone-500",
                    ].join(" ")}
                  >
                    <span>Active Endpoints</span>
                    <span className="text-stone-900">
                      {endpoints.filter((e) => e.status === "active").length} / {endpoints.length}
                    </span>
                  </div>
                  <div className="h-1.5 w-full bg-orange-100 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      animate={{
                        width:
                          endpoints.length > 0
                            ? `${(endpoints.filter((e) => e.status === "active").length / endpoints.length) * 100}%`
                            : "0%",
                      }}
              ...
              <div className="mt-12 pt-8 border-t border-orange-100 flex items-center gap-3">
                <div
                  className={[
                    "w-10 h-10 bg-white rounded-xl flex items-center justify-center border",
                    "border-orange-100",
                  ].join(" ")}
                >
              ...
              <div className="space-y-3">
                {["Python SDK", "Node.js Client", "C++ Engine", "Swift / iOS"].map((lib, i) => (
                  <button
                    key={i}
                    className={[
                      "w-full flex justify-between items-center p-3 bg-stone-50 rounded-xl text-xs font-bold",
                      "text-stone-600 hover:text-stone-900 hover:bg-stone-100 transition-all",
                    ].join(" ")}
                  >
                    {lib}
