  ...
  const features = [
    {
      ...
      desc: [
        "Instantly deploy your model to an infinitely scalable Kubernetes cluster with",
        "auto-scaling APIs instantly generated for you.",
      ].join(" "),
      icon: <Cloud className="w-6 h-6 text-purple-600" />,
    },
    {
      title: "Edge Docker Containers",
      desc: [
        "Export your model as a highly optimized, lightweight Docker container ready to",
        "run on local edge devices, factory floors, or disconnected environments.",
      ].join(" "),
    ...
    },
    {
      ...
      desc: [
        "Every model automatically receives a secure, RESTful API endpoint. Send images",
        "via curl and get JSON bounding boxes back in milliseconds.",
      ].join(" "),
    ...
    },
    {
      ...
      desc: [
        "Test your live models immediately through a web interface. Upload a picture or",
        "stream your webcam right from the browser to verify results.",
      ].join(" "),
    ...
    <div className="bg-[#fcfaf7] min-h-screen">
      <section className="pt-32 pb-20 px-6 lg:px-8 max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <div
            className={[
              "inline-flex items-center gap-2 px-4 py-2 bg-purple-100 border border-purple-200",
              "rounded-full mb-8",
            ].join(" ")}
          >
          ...
          <h1 className="text-5xl lg:text-7xl font-bold text-stone-900 mb-6 tracking-tight">
            Infinite <br />
            <span
              className={["bg-gradient-to-r from-purple-600 to-fuchsia-500 bg-clip-text", "text-transparent"].join(" ")}
            >
              Deployment
            </span>
          </h1>
          <p className="text-xl text-stone-500 max-w-2xl leading-relaxed mb-10">
            A model is useless if it&apos;s stuck in a notebook. VisioX bridges the gap between data science and
            production, allowing you to deploy world-class APIs without DevOps overhead.
          </p>
          <div className="flex gap-4">
            <button
              className={[
                "px-8 py-4 bg-purple-600 hover:bg-purple-700 transition-colors text-white font-bold",
                "rounded-2xl shadow-xl shadow-purple-600/20",
              ].join(" ")}
            >
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {features.map((f, i) => (
              <motion.div
                ...
                className={[
                  "group p-8 rounded-3xl bg-stone-50 border border-stone-100 hover:border-purple-200",
