  ...
  const features = [
    {
      title: "AutoML Tuning",
      desc: [
        "Push-button model deployment. We automatically search hyperparameters and",
        "architectures (YOLO, ResNet, ViT) to find the best fit for your data.",
      ].join(" "),
    ...
    },
    {
      ...
      desc: [
        "Compile your trained models directly for edge hardware like NVIDIA Jetson,",
        "Coral TPU, or iOS CoreML with zero loss in accuracy.",
      ].join(" "),
      icon: <Cpu className="w-6 h-6 text-emerald-600" />,
    },
    {
      title: "Distributed GPU Training",
      desc: "Leverage our scalable cloud GPU clusters. Train massive datasets in hours, not weeks.",
      icon: <Zap className="w-6 h-6 text-emerald-600" />,
    },
    {
      title: "Real-time Metrics",
      desc: "Live visualization of training loss, mAP, and precision/recall curves directly in your web dashboard.",
    ...
    <div className="bg-[#fcfaf7] min-h-screen">
      <section className="pt-32 pb-20 px-6 lg:px-8 max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <div
            className={[
              "inline-flex items-center gap-2 px-4 py-2 bg-emerald-100 border border-emerald-200",
              "rounded-full mb-8",
            ].join(" ")}
          >
          ...
          <h1 className="text-5xl lg:text-7xl font-bold text-stone-900 mb-6 tracking-tight">
            Advanced <br />
            <span
              className={["bg-gradient-to-r from-emerald-600 to-teal-500 bg-clip-text", "text-transparent"].join(" ")}
            >
              Model Training
            </span>
          </h1>
          <p className="text-xl text-stone-500 max-w-2xl leading-relaxed mb-10">
            Train state-of-the-art computer vision models with zero code, or drop down to custom PyTorch workflows.
            VisioX makes multi-GPU training accessible to every engineer.
          </p>
          <div className="flex gap-4">
            <button
              className={[
                "px-8 py-4 bg-emerald-600 hover:bg-emerald-700 transition-colors text-white font-bold",
                "rounded-2xl shadow-xl shadow-emerald-600/20",
              ].join(" ")}
            >
              Start Training
            </button>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {features.map((f, i) => (
              <motion.div
                ...
                className={[
                  "group p-8 rounded-3xl bg-stone-50 border border-stone-100 hover:border-emerald-200",
