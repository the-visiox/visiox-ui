...
import { bboxFromPoints, buildLabelMetaMap, clamp, colorFromMap, labelNameFromMap } from "@/lib/annotation";
...
const AnnotationEditor: React.FC<AnnotationEditorProps> = ({
  ...
  const transformTargetShape = useMemo(
    () =>
      shapes.find((shape) => shape.clientId === transformTargetId && !hiddenShapeIds.includes(shape.clientId)) ?? null,
  ...
  const openShapeClassMenu = useCallback(
    (clientId: string, e: KonvaEventObject<MouseEvent | PointerEvent>) => {
      if (activeTool !== "select") return;
  ...
  const pathVertexEditingEnabled =
    (activeTool === "select" || activeTool === "polygon" || activeTool === "polyline") &&
  ...
  return (
    ...
    >
      <div
        ref={containerRef}
        className={[
          "relative h-full w-full overflow-hidden border border-stone-200/80",
          "bg-stone-200/40 shadow-inner",
        ].join(" ")}
      >
        <div
          className={[
            "absolute right-3 top-3 z-[120] flex flex-col gap-1 rounded-2xl border",
            "border-stone-200/90 bg-white/95 p-1.5 shadow-lg shadow-stone-300/40 backdrop-blur-sm",
          ].join(" ")}
        >
          <button
            type="button"
            ...
            onClick={fitToWindow}
            className={[
              "flex h-10 w-10 items-center justify-center rounded-xl text-stone-600 transition",
        ...
        >
          <Layer ref={layerRef} scaleX={layerScale} scaleY={layerScale} x={layerX} y={layerY}>
            ...
            {shapes.map((shape) => {
              ...
              const topmostVertexIdx =
                shape.shapeType === "polygon" && shape.points && shape.points.length >= 2
                  ? (() => {
                      let minY = Infinity;
                      let minIdx = 0;
                      for (let i = 0; i < shape.points.length / 2; i++) {
                        if (shape.points[i * 2 + 1] < minY) {
                          minY = shape.points[i * 2 + 1];
                          minIdx = i;
                        }
                      }
                      return minIdx;
              ...
              if (shape.shapeType === "tag") {
                ...
                return (
                  <React.Fragment key={shape.clientId}>
                    ...
                    />
                    <Text
                      x={shapeX + 12 / safeLayerScale}
                      y={shapeY + 7 / safeLayerScale}
                      text={shapeLabel}
                      fontSize={11 / safeLayerScale}
                      fontStyle="bold"
                      fill="#ffffff"
                      listening={false}
                    />
              ...
              if (shape.shapeType !== "rectangle" && shape.points && shape.points.length >= 2) {
                ...
                return (
                  <React.Fragment key={shape.clientId}>
                    ...
                    {(isPointsShape || isSelected || (pathVertexEditingEnabled && hoveredId === shape.clientId)) &&
        ...
        {contextMenu && (
          <div
            className={[
              "absolute z-[140] min-w-[12rem] rounded-2xl border border-stone-200/90 bg-white/95 p-2",
              "shadow-2xl shadow-stone-300/40 backdrop-blur-md",
            ].join(" ")}
            style={{
              left: Math.max(12, Math.min(contextMenu.x + MENU_GAP, Math.max(12, dimensions.width - MENU_WIDTH - 12))),
              top: Math.max(12, Math.min(contextMenu.y + MENU_GAP, Math.max(12, dimensions.height - MENU_HEIGHT - 12))),
            }}
          >
            <div className="px-2 pb-2 text-[10px] font-bold uppercase tracking-widest text-stone-500">Class</div>
            <div className="space-y-1">
              {labels.map((label) => (
                <button
                  ...
                  className={[
                    "flex w-full items-center gap-2 rounded-xl px-2 py-2 text-left text-xs font-semibold",
