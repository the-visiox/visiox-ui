...
export default function SolutionPageTemplate({ config }: { config: SolutionPageConfig }) {
  ...
  return (
    <div className={`min-h-screen ${bgTint} overflow-x-hidden`}>
      ...
      <section
        className={[
          "relative pt-32 pb-20 px-6 lg:px-8 bg-white overflow-hidden min-h-[85vh] flex flex-col",
          "justify-center",
        ].join(" ")}
      >
        {/* Background ambience */}
        <div className="absolute inset-0 pointer-events-none">
          <div
            className={[
              "absolute top-0 right-0 w-[700px] h-[700px] rounded-full blur-[120px] -translate-y-1/2",
              "translate-x-1/2 opacity-30",
            ].join(" ")}
            style={{ backgroundColor: accentHex }}
          />
          <div
            className={[
              "absolute bottom-0 left-0 w-[500px] h-[500px] bg-stone-100/50 rounded-full blur-[100px]",
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex items-center gap-2 mb-6">
                <Link
                  href="/solutions"
                  className={["text-sm font-medium text-stone-500 hover:text-stone-900", "transition-colors"].join(" ")}
                >
                  Solutions
                </Link>
                <span className="text-stone-300">/</span>
                <span className="text-sm font-bold uppercase tracking-wider" style={{ color: accentHex }}>
                  {badge}
                </span>
              </div>
              ...
              <h1
                className={["text-5xl lg:text-7xl font-bold text-stone-900 mb-8 leading-[1.1]", "tracking-tight"].join(
                  " ",
                )}
              >
                {title}
              ...
              <p className="text-xl text-stone-600 mb-10 leading-relaxed max-w-xl">{subtitle}</p>

              <div className="flex flex-col sm:flex-row gap-4">
                <button
                  className={[
                    "px-10 py-4 text-white rounded-2xl font-bold transition-all shadow-xl",
                ...
                <button
                  className={[
                    "px-10 py-4 bg-white text-stone-900 border border-stone-200 rounded-2xl font-bold",
            ...
            >
              <div
                className={["aspect-[4/3] rounded-3xl overflow-hidden shadow-2xl border-4", "border-white group"].join(
                  " ",
                )}
              >
                <Image
      ...
      <section className="bg-white px-6 lg:px-8 pt-16 pb-24 relative z-20">
        <div className="max-w-7xl mx-auto">
          ...
          <div
            className={[
              "rounded-[2.5rem] shadow-xl shadow-stone-200/50 py-8 px-6 lg:py-10 lg:px-12 mb-24",
          ...
          >
            ...
            <div
              className={[
                "absolute top-0 right-0 w-64 h-64 bg-white/20 rounded-full blur-[80px] -translate-y-1/2",
                "translate-x-1/2 pointer-events-none",
              ].join(" ")}
            />
            <div
              className={[
                "absolute bottom-0 left-0 w-64 h-64 bg-black/10 rounded-full blur-[80px] translate-y-1/2",
                "-translate-x-1/2 pointer-events-none",
              ].join(" ")}
            />
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 text-center relative z-10">
              {stats.map((stat, i) => (
                <div key={i} className="space-y-3 group">
                  <div
                    className={[
                      "mx-auto w-12 h-12 bg-white/15 rounded-2xl flex items-center justify-center",
                      "group-hover:scale-110 transition-transform text-white",
                    ].join(" ")}
                  >
                    {stat.icon}
                  </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {capabilities.map((item, i) => (
              <div
                key={i}
                className={[
                  "p-8 bg-stone-50/50 rounded-3xl border border-stone-100 hover:bg-white hover:shadow-2xl",
                  "hover:border-stone-200 transition-all duration-300 group",
                ].join(" ")}
              >
                <div
                  className={[
                    "mb-6 w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm",
      ...
      <section className="pt-24 pb-8 md:pb-16 px-6 lg:px-8 bg-stone-50 overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            ...
            <Link
              href="/contact"
              className="inline-flex items-center font-bold group transition-all"
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            {useCases.map((useCase, i) => (
              <div
                key={i}
                className={[
                  "group flex flex-col bg-white rounded-3xl overflow-hidden border border-stone-200",
                  "hover:border-stone-300 hover:shadow-xl transition-all",
                ].join(" ")}
              >
                {/* Image */}
                <div className="relative h-56 overflow-hidden">
                  ...
                  <div
                    className={[
                      "absolute inset-0 bg-gradient-to-t from-black/80 via-transparent",
                      "to-transparent",
                    ].join(" ")}
                  />
      ...
      <section className="pt-8 md:pt-16 pb-12 md:pb-24 px-6">
        <div
          className={[
            "max-w-4xl mx-auto rounded-3xl p-8 lg:p-12 text-center text-white",
        ...
        >
          <div
            className={[
              "absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-[80px] -translate-y-1/2",
              "translate-x-1/2",
            ].join(" ")}
          />
          <div
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/contact"
                className={[
                  "px-8 py-3.5 bg-white rounded-xl font-bold hover:bg-stone-50 transition-all shadow-xl",
                  "hover:-translate-y-0.5",
                ].join(" ")}
                style={{ color: accentHex }}
              >
              ...
              <Link
                href="/contact"
                className={[
                  "px-8 py-3.5 bg-white/10 text-white border border-white/30 rounded-xl font-bold",
