...
export default function Header() {
  ...
  return (
    <motion.nav
      ...
      className={["fixed top-0 left-0 right-0 z-50 bg-white/70 backdrop-blur-md border-b", "border-stone-200"].join(
        " ",
      )}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <Link href="/">
              <span
                className={["text-stone-600 hover:text-[#FF7300] transition-colors cursor-pointer", "font-medium"].join(
                  " ",
                )}
              >
                Home
            ...
            <div className="relative group" onMouseEnter={() => setActiveDropdown("product")}>
              <span
                className={[
                  "text-stone-600 hover:text-[#FF7300] transition-colors cursor-pointer font-medium flex",
                  "items-center gap-1",
                ].join(" ")}
              >
                Product
              ...
              {activeDropdown === "product" && (
                <motion.div
                  ...
                  className={[
                    "absolute top-full -left-4 w-48 bg-white border border-stone-200 shadow-xl rounded-xl p-2",
                    "mt-2",
                  ].join(" ")}
                >
                  {productLinks.map((link) => (
                    <Link key={link.name} href={link.href}>
                      <span
                        className={[
                          "block px-4 py-2 text-sm text-stone-600 hover:bg-orange-50 hover:text-[#FF7300]",
            ...
            <div className="relative group" onMouseEnter={() => setActiveDropdown("solution")}>
              <Link href="/solutions">
                <span
                  className={[
                    "text-stone-600 group-hover:text-[#FF7300] transition-colors cursor-pointer font-medium",
                    "flex items-center gap-1",
                  ].join(" ")}
                >
                  Solution
                  <svg
                    className={`w-4 h-4 transition-transform ${activeDropdown === "solution" ? "rotate-180" : ""}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              {activeDropdown === "solution" && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className={[
                    "absolute top-full -left-4 w-72 bg-white border border-stone-200 shadow-xl rounded-xl p-2",
                    "mt-2",
                  ].join(" ")}
                >
          ...
          >
            {isLoggedIn ? (
              <Link href="/home">
                <span
                  className={[
                    "hidden md:block text-[#FF7300] hover:text-stone-900 transition-colors font-bold",
                    "cursor-pointer",
                  ].join(" ")}
                >
                  Go to Workspace
              ...
              <Link href="/login">
                <span
                  className={[
                    "hidden md:block text-stone-600 hover:text-[#FF7300] transition-colors",
                    "cursor-pointer",
                  ].join(" ")}
                >
                  Sign In
