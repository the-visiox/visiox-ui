...
const EXPORT_LABELS = new Map<ExportFormat, string>(
  EXPORT_FORMAT_GROUPS.flatMap((group) =>
    group.formats.map((format) => [format.value, format.label] as const),
  ),
);
...
export default function DatasetExportDialog({
  ...
  const [exportContent, setExportContent] =
    useState<ExportContent>("images-and-labels");
  const [downloadOption, setDownloadOption] =
    useState<DownloadOption>("download");
  ...
  const codeSnippet = selectedFormat
    ? targets.length === 1
      ? [
          ...
          `params = {"export_format": "${selectedFormat}", "save_images": "${exportContent === "images-and-labels" ? "1" : "0"}"}`,
          "",
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      for (const { target } of exportItems) {
        const blob = await datasets.exportArchive(
          target.id,
          selectedFormat,
          exportContent === "images-and-labels",
        );
      ...
      if (!(error instanceof DOMException && error.name === "AbortError")) {
        setExportError(
          error instanceof Error ? error.message : "Dataset export failed.",
        );
      }
    } finally {
      setExporting(false);
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[250] flex items-center justify-center bg-stone-950/45 p-4 backdrop-blur-sm"
      onMouseDown={(event) => {
    ...
    >
      <motion.div
        ...
        className="w-full max-w-2xl overflow-visible rounded-2xl border border-stone-200 bg-[#fcfaf7] shadow-2xl shadow-stone-950/20"
      >
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs font-medium text-stone-500">
                {subtitle ??
                  (targets.length === 1
                    ? targets[0]?.name
                    : `${exportName} - ${targets.length} datasets`)}
              </p>
            </div>
          </div>
          <button
            ...
            onClick={closeDialog}
            disabled={exporting}
            className="rounded-xl p-2 text-stone-400 transition hover:bg-stone-100 hover:text-stone-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400/50 disabled:cursor-not-allowed disabled:opacity-40"
        ...
        {step === "options" ? (
          <>
            <div className="max-h-[calc(100vh-10rem)] space-y-7 overflow-y-auto px-5 py-6 sm:px-6">
              <div>
                ...
                <div ref={formatMenuRef} className="relative">
                  <button
                    ...
                    disabled={exporting}
                    className={`flex h-11 w-full items-center justify-between rounded-xl border bg-white px-3.5 text-left text-sm font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400/35 ${
                      formatMenuOpen
                  ...
                  >
                    <span
                      className={
                        selectedFormat ? "text-stone-800" : "text-stone-500"
                      }
                    >
                      {selectedFormat
                        ? EXPORT_LABELS.get(selectedFormat)
                        : "Select a format"}
                    </span>
                    <ChevronDown
                  ...
                  <AnimatePresence>
                    {formatMenuOpen ? (
                      <motion.div
                        ...
                        className="absolute left-0 right-0 top-[calc(100%+0.45rem)] z-20 max-h-72 overflow-y-auto rounded-xl border border-stone-200 bg-white p-1.5 shadow-xl shadow-stone-300/40"
                      >
                        {EXPORT_FORMAT_GROUPS.map((group) => (
                          ...
                          >
                            <p className="px-2.5 pb-1 pt-1.5 text-[10px] font-extrabold uppercase tracking-[0.16em] text-stone-400">
                              {group.label}
                            </p>
                            {group.formats.map((format) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-left text-sm font-semibold transition ${
                                    selected
              ...
              <fieldset disabled={exporting} className="disabled:opacity-60">
                <legend className="mb-2.5 text-sm font-bold text-stone-800">
                  Export contents
                </legend>
                <div className="grid gap-2 sm:grid-cols-2">
                  <label
                    className={`flex min-h-[88px] cursor-pointer items-start gap-3 rounded-xl border px-4 py-3.5 transition focus-within:ring-2 focus-within:ring-orange-400/30 ${
                      exportContent === "images-and-labels"
              ...
              <fieldset disabled={exporting} className="disabled:opacity-60">
                ...
                <div className="space-y-2">
                  <label
                    className={`flex min-h-[76px] cursor-pointer items-start gap-3 rounded-xl border px-4 py-3.5 transition focus-within:ring-2 focus-within:ring-orange-400/30 ${
            ...
            <div className="flex items-center justify-between gap-3 border-t border-stone-200 bg-white/70 px-5 py-3.5 sm:px-6">
              <button
                type="button"
                onClick={closeDialog}
                disabled={exporting}
                className="h-10 rounded-xl border border-stone-300 bg-white px-4 text-sm font-bold text-stone-600 transition hover:bg-stone-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400/40 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                className="flex h-10 items-center rounded-xl bg-gradient-to-r from-[#E66700] via-[#FF7300] to-[#F1A222] px-5 text-sm font-bold text-white shadow-lg shadow-orange-500/20 disabled:pointer-events-none disabled:opacity-45"
              >
          ...
          <>
            <div className="px-5 py-6 sm:px-6">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-stone-500">
                    Replace the access-token placeholder before running.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => void handleCopy()}
                  className="flex h-9 shrink-0 items-center gap-2 rounded-xl border border-stone-200 bg-white px-3 text-xs font-bold text-stone-600 transition hover:bg-stone-50"
                >
                  {copied ? (
                    <CheckCircle2
              ...
              <pre className="max-h-80 overflow-auto rounded-2xl bg-stone-950 p-4 text-xs leading-6 text-stone-100 shadow-inner">
            ...
            <div className="flex items-center justify-between gap-3 border-t border-stone-200 bg-white/70 px-5 py-3.5 sm:px-6">
              <button
                ...
                className="h-10 rounded-xl border border-stone-300 bg-white px-4 text-sm font-bold text-stone-600 transition hover:bg-stone-50"
              >
                Back
              </button>
              <button
                type="button"
                onClick={closeDialog}
                className="h-10 rounded-xl bg-stone-900 px-5 text-sm font-bold text-white transition hover:bg-stone-800"
              >
                Done
              </button>
            </div>
